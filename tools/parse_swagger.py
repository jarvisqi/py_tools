...
urls = [
    # 'http://xxx.xxx.com/customer/v2/api-docs',
    # 'http://xxx.xxx.com/product/v2/api-docs',
    # 'http://xxx.xxx.com/order/v2/api-docs',
    # 'http://xx.xxx.com/pay/v2/api-docs'
    ...
    for url in urls:
        ...
        for svc, data in svc_path.items():
            ...
            if req_method == "post":
                ...
                if "schema" in params[0]:
                    # 参数是对象
                    if "$ref" in params[0]["schema"]:
                        cls_name = params[0]["schema"]["$ref"].split("/")[2]
                        ...
                        for name, _ in fileds.items():
                            if name == "orderBy":
                                ...
                                parameter = Parameter(
                                    name, fileds[name]["type"], fileds[name]["allowEmptyValue"], fileds[name]["description"])

                            parameter_list.append(parameter)

                    # 参数是数组
                    if "$items" in params[0]["schema"]:
                        parameter = Parameter(params[0]["name"],
                                              params[0]["type"], params[0]["required"], params[0]["description"])

                        parameter_list.append(parameter)
            else:
                response_result = data.get("get")["responses"]
                # get请求
                if "parameters" in data.get("get"):
                    params = data.get("get")["parameters"]
                  # 参数是数组
                    for p in params:
                        if "schema" in p:
                            parameter = Parameter(p["name"],
                                                  "array", p["required"], p["description"])
                            parameter_list.append(parameter)
            ...
            result_str = response_result["200"]["schema"]["$ref"].split(
                "/")[2]
            # 返回类型
            result = pase_response(result_str, svc_definitions)

            api = Api(summary, svc, req_method,
                      "application/json", "json", parameter_list, result)
            api_list.append(api)
...
def sava_doc():
    ...
    write_excel(wb, "pay_doc", api_list)
    print("finished",
          "\n=========================================================")
...
if __name__ == "__main__":
    ...
    sava_doc()
    # # resu="StandResponse«PageResult«AccountBalanceDetailFixDTO»»"
    # resu="StandResponse«AccountBalanceDetailFixDTO»"
    # if resu.count("«") >2:
    #     cls_name=resu.split("«")[2][:-2]
    # else:
    #     cls_name=resu.split("«")[1][:-1]
    # print(cls_name)
