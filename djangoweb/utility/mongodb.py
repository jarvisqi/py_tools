...
class MongoDBManager(object):
    ...
    def update(self, table, conditions, value, s_upsert=False, s_multi=False):
        ...
        self.db[table].update(
            conditions, value, upsert=s_upsert, multi=s_multi)
...
def main():
    db = MongoDBManager()
    # save
    mongo_db.save("table", {"_id": 102, "data": 102})

    # find_one
    db.find_one("table", {"_id": 101})

    # find 单个个值 查询数据 倒序
    db.find("table", {"data": 101}).sort([("_id", -1)])
    # find 多个值 查询数据   倒序   获取第一个
    db.find("table", {"data": {"$in": [1, 2, 3, 4]}}).sort([("_id", -1)]).limit(1)
